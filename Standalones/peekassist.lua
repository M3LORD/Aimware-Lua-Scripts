...
local reference = gui.Reference("RAGE", "MAIN", "Aimbot");
local keybox = gui.Keybox(reference, "nex_peekassist", "Quick Peek Assist", 0);
    ...
    if(keybox:GetValue() ~= 0 and input.IsButtonDown(keybox:GetValue())) then
        if(inverse) then
            ...
            if(FinalVelocity >= 10) then
                if(CMD:GetSideMove() > 1) then
                    CMD:SetSideMove(-450);
                elseif(CMD:GetSideMove() < -1) then
                    CMD:SetSideMove(450);
                end
            else
                inverse = false;
